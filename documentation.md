...
2. **DeepSeek Flash connector — completed.** The worker calls the OpenAI-compatible DeepSeek chat-completions endpoint with `deepseek-v4-flash`, JSON mode, a 90-second request timeout, bounded retries, schema validation through Zod, and corrective retry instructions when a response shape is invalid.
